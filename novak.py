...
def get_kovansports_availability(check_date, check_time_start, check_time_end):
    ...
    time_start = int(check_time_start.translate(None, ':'))   #filter results based on start time- must be integer- without leading 0
    time_end = int(check_time_end.translate(None, ':'))     #filter results based on end time- must be integer- without leading 0
    check_list = []
    a=time_start
    while a<time_end:
        check_list.append(a)
