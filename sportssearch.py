...
class timeslots(object):
    ...
    def search(self):
        ...
        pool.apply_async(get_kovansports_availability, args=(check_date, check_time_start, check_time_end), callback=self.print0) ,
        pool.apply_async(get_zionsports_availability, args=(check_date, check_time_start, check_time_end), callback=self.print0)  ,
        pool.apply_async(get_kallangcage_availability, args=(check_date, check_time_start, check_time_end), callback=self.print0) ,
        pool.apply_async(get_bttimahcage_availability, args=(check_date, check_time_start, check_time_end), callback=self.print0) ,
        pool.apply_async(get_hyfa_availability, args=(check_date, check_time_start, check_time_end), callback=self.print0),
        ]
        pool.close()
        pool.join()
        if not self.results:
